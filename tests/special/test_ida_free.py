...
async def test_get_latest_ida_free_package_success(mocker: MockerFixture) -> None:
    ...
    result = await get_latest_ida_free_package('dev-util/ida-free-9.2', mock_settings)

    mock_get_content.assert_called_once_with('https://docs.hex-rays.com/release-notes/')
    assert result == '9.3'


async def test_get_latest_ida_free_package_no_content(mocker: MockerFixture) -> None:
    """Test when content fetch fails."""
    mocker.patch('livecheck.special.ida_free.get_content', return_value=None)
    mock_settings = mocker.Mock()

    result = await get_latest_ida_free_package('dev-util/ida-free-9.0', mock_settings)

    assert not result


async def test_get_latest_ida_free_package_no_versions_found(mocker: MockerFixture) -> None:
...
async def test_get_latest_ida_free_package_mixed_versions(mocker: MockerFixture) -> None:
...
async def test_get_latest_ida_free_package_invalid_version_format(mocker: MockerFixture) -> None:
...
async def test_get_latest_ida_free_package_all_invalid_versions(mocker: MockerFixture) -> None:
...
async def test_get_latest_ida_free_package_three_part_versions(mocker: MockerFixture) -> None:
...
async def test_get_latest_ida_free_package_only_three_part_versions(mocker: MockerFixture) -> None:
...
async def test_get_latest_ida_free_package_value_error_in_conversion(mocker: MockerFixture) -> None:
