...
async def test_get_latest_repology_success(mock_catpkg_catpkgsplit: Mock, mocker: MockerFixture,
    ...
    result = await repology.get_latest_repology('cat/pkg', mock_settings)
    assert result == '1.2.3'
    mock_get_content.assert_called_once()
    mock_get_last_version.assert_called_once()


async def test_get_latest_repology_with_package_param(mock_catpkg_catpkgsplit: Mock,
    ...
    result = await repology.get_latest_repology('cat/pkg', mock_settings, package='custom-pkg')
    assert result == '4.5.6'
    mock_get_content.assert_called_once()
    mock_get_last_version.assert_called_once()


async def test_get_latest_repology_fallback_url(mock_catpkg_catpkgsplit: Mock,
...
async def test_get_latest_repology_no_content(mock_catpkg_catpkgsplit: Mock, mocker: MockerFixture,
                                              mock_settings: Mock) -> None:
    mock_get_content = mocker.patch('livecheck.special.repology.get_content')
...
async def test_get_latest_repology_no_matching_release(mock_catpkg_catpkgsplit: Mock,
                                                       mocker: MockerFixture,
                                                       mock_get_last_version: Mock,
                                                       mock_settings: Mock) -> None:
    mock_response = mocker.Mock()
...
async def test_get_latest_repology_devel_status_allowed(mock_catpkg_catpkgsplit: Mock,
                                                        mocker: MockerFixture,
                                                        mock_get_last_version: Mock,
                                                        mock_settings: Mock) -> None:
