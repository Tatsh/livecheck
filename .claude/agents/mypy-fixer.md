...
Fixes mypy type errors and eliminates `Any` usage in the livecheck project.
...
1. Fix all mypy errors reported by `uv run mypy livecheck`
...
Place new TypedDict classes in `livecheck/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`livecheck/typing.py` when shared.
...
1. Run `uv run mypy livecheck` and capture all errors.
...
4. Run `uv run mypy livecheck` again to verify fixes.
