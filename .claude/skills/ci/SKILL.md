...
If any changed files are under `livecheck/` or `tests/`, run the following
...
1. **click-auditor** - validate Click command consistency. **Only run if the project directly uses
   Click and files under `livecheck/commands/` changed.**
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `livecheck/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `livecheck/commands/main.py`,
...
For Python files, strip the `livecheck/` prefix and replace `/` with `.` (like module imports).

- Python file `livecheck/media.py` → `media:`.
- Multiple files under `livecheck/commands/` → `commands:`.
- Single command file `livecheck/commands/admin.py` → `commands.admin:`.
