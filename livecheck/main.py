...
def _package_src_uris(src_uris: Sequence[str], pkg: str, ebuild_version: str) -> tuple[str, ...]:
    ...
    Vendor archives generated by livecheck and signature or checksum files are skipped. The
    first remaining entry is always selected. Later entries are usually vendored dependencies or
    ...
    or ``_`` and without a ``-bin`` suffix. When every entry is skipped, the first entry is
    selected.
    ...
    """
    candidates = [uri for uri in src_uris if not uri.lower().endswith(_SKIPPED_SRC_URI_SUFFIXES)]
    if not candidates:
        return tuple(src_uris[:1])
    ...
    return (candidates[0], *(uri for uri in candidates[1:]
                             if version_pattern.search(uri) and name_pattern.search(uri.lower())))
